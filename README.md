...
这个仓库已经从单篇 README 扩展成一套 Codex 实战教程站。原来的国内站、备用入口、购买套餐、中转 API 和飞书教程链接继续保留，新增内容主要放在下面几个目录：
...
### 2.6 Codex 多入口地图：不要只盯着一个按钮

很多教程会把 Codex 讲成一个单一工具：要么是 CLI，要么是桌面 App。更准确的理解是：

> **Codex 是一组入口协同的工作系统。**

不同入口适合不同任务，新手先理解这张表，后面会少走很多弯路：

| 入口 | 更适合什么 | 典型任务 |
|------|------------|----------|
| **桌面 App** | **本地多任务工作台** | **管理项目文件夹、开 Thread、用 Skills、跑自动化任务** |
| **CLI** | **贴近终端和本地仓库** | **修 bug、跑命令、补测试、解释项目结构** |
| **Cloud / Web** | **后台长任务和仓库协作** | **连接 GitHub、分析仓库、生成 PR、处理较长任务** |
| **IDE** | **编辑器上下文里的局部修改** | **解释代码、改当前文件、做局部审查** |
| **ChatGPT 手机端 Codex** | **离开电脑后继续跟进任务** | **查看进度、回复问题、审批操作、调整方向** |
| **MCP / 插件生态** | **连接外部工具和数据源** | **浏览器测试、Notion、Figma、飞书、GitHub、文件系统等** |

最简单的选择方法：

- **要频繁看命令输出**：用 CLI
- **要本地多任务、Skills、定时任务**：用桌面 App
- **要后台跑仓库任务、生成 PR**：用 Cloud / Web
- **正在编辑某个文件**：用 IDE
- **人离开电脑但任务不能停**：用 ChatGPT 手机端跟进

> **真正影响效率的不是你用了哪个入口，而是你有没有把任务放到合适的入口里。**

### 2.7 手机端协同：不是远程桌面，而是随身工作台

现在 Codex 也可以通过 **ChatGPT 手机 App** 来跟进任务。这里要说清楚一件事：

> **手机端 Codex 不是单独的手机 Codex App，也不是把手机变成远程桌面鼠标。**

更准确地说，它是让你在手机上连接正在运行的 Codex 环境，继续处理这些事情：

- **查看正在跑的 Thread 和任务进度**
- **阅读 Codex 的阶段性输出、终端日志、截图和 diff**
- **回复 Codex 的澄清问题**
...
-> 手机端查看进度
-> 需要确认时直接回复或审批
-> 回到电脑后再做完整 diff review
```

但高风险操作，最好还是回到电脑上确认，比如：

- **删除文件**
- **发布部署**
- **数据库迁移**
- **支付、认证、权限相关修改**
- **需要长时间阅读大量 diff 的代码合并**

一句话总结：

> **手机端适合跟进和审批，不适合替代最终审查。**
...
Codex 能不能稳定干活，不只取决于模型，也取决于你有没有把任务讲清楚。一个好任务，最好包含 6 个要素：

| 要素 | 写什么 | 示例 |
|------|--------|------|
| **目标** | **一句话说明要得到什么结果** | **修复登录页刷新后状态丢失的问题** |
| **背景** | **说明现象、上下文和为什么要改** | **用户登录后刷新页面会回到未登录状态** |
| **范围** | **限定目录、模块或文件** | **优先检查 `src/auth` 和相关测试** |
...
```text
请先不要修改代码。先阅读相关文件，分析这个任务会影响哪些模块，并输出：
1. 影响面
2. 推荐实施步骤
3. 每一步的验证方式
4. 第一阶段最小改动建议
...
> **小任务直接做，大任务先拆开。先让 Codex 看清边界，再让它动手。**
...
### 6.4 项目级 AGENTS.md：让 Codex 真正懂你的仓库

Personalization 更像全局规则，而 **`AGENTS.md`** 更适合写进具体项目。它的作用是告诉 Codex：
...
Full access 的效率很高，但不代表所有任务都应该无脑放行。建议把任务分成两类：

| 类型 | 可以更快推进 | 建议先确认 |
|------|--------------|------------|
| **低风险任务** | **改文档、补测试、修本地可复现 bug、运行已有测试命令** | **一般确认计划后就能做** |
| **高风险任务** | **删除文件、数据库迁移、发布部署、生产服务、支付/认证/权限、读取敏感数据** | **必须先说明影响，再人工确认** |

可以把这段直接写进全局规则或 `AGENTS.md`：

```text
请在动手前先说明你计划运行的命令和可能影响的文件。
不要读取 .env、密钥、token 或任何私有凭据。
...
- **访问生产数据库或生产服务器**
- **提交表单、付款、改权限**
- **读取 cookie、token、API Key、私有配置**

> **效率和安全不是二选一。正确做法是：低风险自动化，高风险人工确认。**
...
如果只讲安装和写代码，很多小白还是不知道 Codex 到底能干什么。更容易理解的方式，是直接给场景：

| 场景 | Codex 可以怎么用 | 验证重点 |
|------|------------------|----------|
| **PPT / 演示文稿** | **根据主题生成结构、页面文案和初版素材** | **逻辑结构、视觉统一、导出效果** |
| **Obsidian / 知识库** | **整理文章、建立索引、生成选题池、补充引用** | **文件路径、链接、来源是否准确** |
| **浏览器自动化** | **检查网页、填写测试表单、截图验证前端效果** | **页面状态、点击路径、是否误提交** |
| **Figma / 设计协作** | **读取设计稿、提取布局和组件边界** | **设计 token、间距、组件还原度** |
| **Notion / 飞书** | **读取数据库、汇总内容、生成日报或同步资料** | **权限范围、字段映射、写回位置** |
| **GitHub Actions CI** | **分析失败日志、定位原因、提出最小修复** | **失败复现、测试通过、PR 描述清楚** |
| **远程服务器排障** | **读取日志、定位报错、给出修复步骤** | **不碰生产数据，先复现再修复** |

对非程序员来说，最有价值的不是让 Codex “写一段代码”，而是把它接进真实工作流：
...
-> 生成可检查产物
-> 人工确认
-> 沉淀成模板 / Skill / AGENTS.md
```

> **一次任务做成结果，多次任务沉淀流程。Codex 真正值钱的地方，是让你的工作方法可复用。**
