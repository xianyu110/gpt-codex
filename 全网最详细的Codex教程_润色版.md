...
这就是为什么最近美股那边天天吵着说"软件的逻辑变了"——一个Claude的Excel cowork插件能给华尔街吓个半死的原因。

### 1.3 模型对比

Codex和Claude Code都是上层的编程应用，应用本身需要搭配模型一起使用。正好上周五两家都发了自己的新模型：
...
| 2. 配置中转API | 每天100刀额度 | https://maynorai.tqfk.xyz/item/15 |
...
> 💡 **提示**：Windows应用版本目前还没上线，只能使用命令行版本，不过应用版本应该也快了，预计就在这一周内。
...
> ⚠️ **建议**：日常使用 **High**，遇到超出日常范畴的难活、硬活、大活时，使用 **Extra High**（基本等同于5.2的`xhigh`）。**这种大活，成功率和时间成本才是最值钱的，多出来的推理消耗可以忽略。**
...
Coding这块反而没啥可以说的了，**直接用嘴描述你的需求就行了**！
